...
**Files:**
- Modify: `flow-view.js`, `index.html` (CSS), `flow-core.js` (turrell geometry)
- Test: `tests/flow.spec.js` (append), `tests/flow-core.spec.js` (append)

**Turrell display mapping (approved via mockup review 2026-07-05):** turrell renders
stop `pos=0` as the *outermost* square (matching Rate Views thumbnails) and its
squares center at 50%/50% — unlike radial/conic which center at 42%. Two rules:

1. In `flow-core.js`, give turrell its own axis branch centered at `CY_TURRELL = 0.5`
   (same center→edge line otherwise). Update `axisPoint`/`axisPos`/`offAxisDistance`
   turrell cases and append unit tests:
...
2. In `flow-view.js` edit rendering, map turrell handle display position as
   `1 - pos` (handle sits ON its square: innermost = center), and invert on drag:

```js
const dispT = (pos) => _mode() === 'turrell' ? 1 - pos : pos;
const dragPos = (x, y, w, h) => {
  const t = axisPos(_mode(), x, y, w, h);
  return _mode() === 'turrell' ? 1 - t : t;
};
```

Use `dispT(s.pos)` everywhere `axisPoint(_mode(), s.pos, …)` is called for handles,
labels, and midpoints (midpoint display = `dispT(midpoint)`), and `dragPos(...)`
instead of raw `axisPos(...)` in `_onStopDown`'s move handler (Task 8).
